...
//        WeatherFromLib360 weatherFromLib360 = UpdatedActivity.weatherFromLib360;
        ...
        WeatherInfo.HeWeather heWeather = MyApplication.getWeatherInfo().getHeWeathers().get(0);
        for (int i = 0; i < heWeather.getHourly_forecast().size(); i++) {
            tempMax[i] = Float.parseFloat(
                    heWeather.getHourly_forecast().get(i).getTmp());
            tempMin[i] = Float.parseFloat(
                    heWeather.getHourly_forecast().get(i).getHum());
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Xstartx = viewWidth / 10;
        Xstopx = viewWidth * 9 / 10;
        Xy = viewHight * 9 / 10;
        Ystarty = viewHight / 10;
        Ystopy = Xy;
        Yx = Xstartx;
